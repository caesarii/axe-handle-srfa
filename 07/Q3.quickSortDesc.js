...
if(require.main === module) {
    ...
    function testParttion2() {
        ...
        ensure(partition(list, 0, list.length - 1) === 2, 'partion test 2')
    }

    function testParttion3() {
        const list = [0, 1, 2]
        ensure(partition(list, 0, list.length - 1) === 0, 'partion test 3')
    }

    function testParttion4() {
        const list = [1, 2, 0]
        ensure(partition(list, 0, list.length - 1) === 2, 'partion test 4')
    }

    function testParttion5() {
        const list = [0, 1]
        ensure(partition(list, 0, list.length - 1) === 0, 'partion test 5')
    }

    function testParttion6() {
        const list = [1, 0]
        ensure(partition(list, 0, list.length - 1) === 1, 'partion test 6')
    }
    ...
    function testParttion8() {
        const list = [0, 1, 2, 3, 5, 6, 4]
        ensure(partition(list, 3, list.length - 1) === 5, 'partion test 8')
    }

    function testQuickSort1 () {
        const list = [38, 545, 6, 9, 324, 1, 4, 564, 17, 754]
        quickSort(list)
        ensure(equalArray(list, [754, 564, 545, 324, 38, 17, 9, 6, 4, 1]), 'quick sort test 1')
    }
    ...
    function testQuickSort2 () {
        ...
        quickSort(list)
            ...
            equalArray(list, [8, 7, 6, 5, 4, 3, 2, 1 ]),
            'quick sort test 2'
        )
    }
